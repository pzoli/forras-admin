...
public class ScheduleService implements Serializable {
	...
	public List<ScheduleEvent> loadEvents(Date start, Date end, TimeZone timeZone, boolean isMeeting,
			SystemUser[] findUsers) {
		...
		String select = "from EventHistory e where e.startDate between :beginDate and :endDate and styleClass is "
				+ (isMeeting ? "not null" : "null");

		if ((findUsers != null)) {
			if (findUsers.length > 0) {
				select += " and e.createdBy in (:createdBy) or e.eventId in (select e2.eventId from EventHistory e2 join e2.leaders l where styleClass is "
						+ (isMeeting ? "not null" : "null")
						+ " and e2.startDate between :beginDate and :endDate and l in (:users))";
			} else {
	...
	/*
	 * public Session getSession() { Session session = (Session) em.getDelegate();
	 * return session; }
	 */
